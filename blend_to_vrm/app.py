...
======================
Flask-Service zum Konvertieren von Blender .blend Dateien zu VRM.

Features:
  - ./blend/ Ordner: ZIP-Dateien mit .blend + Texturen ablegen -> automatische Konvertierung
  - Upload via Web-UI oder API (ZIP oder .blend)
...
Nutzung:
  1. python app.py
  2. ZIP-Dateien (mit .blend + Texturen) in ./blend/ legen
  3. Browser: http://localhost:5011 -> automatisch konvertiert
"""
...
SERVER_PORT = CONFIG.get("port", 5011)
BLENDER_PATH = CONFIG.get("blender_path", r"C:\Program Files\Blender Foundation\Blender 4.2\blender.exe")
BLEND_DIR = os.path.join(SCRIPT_DIR, "blend")    # Input: ZIPs mit .blend + Texturen
...
for _d in [BLEND_DIR, VRM_DIR, UPLOAD_DIR, WORK_DIR]:
...
def extract_zip(zip_path: str, job_id: str) -> str:
    """
    ZIP entpacken -> .blend Datei + Texturen in Work-Verzeichnis.
    Gibt den Pfad zur .blend Datei zurueck.
    """
    ...
    # .blend Datei finden (auch in Unterordnern)
    blend_file = None
    ...
    if not blend_file:
        raise RuntimeError("Keine .blend Datei in der ZIP gefunden!")
    ...
    add_job_log(job_id, f"Blend-Datei: {os.path.basename(blend_file)}")
    add_job_log(job_id, f"Texturen gefunden: {tex_count}")

    return blend_file


def run_conversion(job_id: str, blend_path: str, output_dir: str = None, cleanup_dir: str = None, options: dict = None):
    ...
    try:
        ...
        output_name = os.path.splitext(os.path.basename(blend_path))[0] + ".vrm"
        ...
        vrm_ver = options.get('vrm_version', '1.0')
        add_job_log(job_id, f"VRM-Version: {vrm_ver}")

        add_job_log(job_id, f"Blend-Datei: {os.path.basename(blend_path)}")
...
def health():
    ...
    blend_zips = [f for f in os.listdir(BLEND_DIR) if f.lower().endswith(".zip")] if os.path.exists(BLEND_DIR) else []
    vrm_files = [f for f in os.listdir(VRM_DIR) if f.lower().endswith(".vrm")] if os.path.exists(VRM_DIR) else []
    return jsonify({
        "status": "ok",
        "service": "blend_to_vrm",
        "blender_found": blender_ok,
        "blender_path": BLENDER_PATH,
        "blend_folder": len(blend_zips),
...
def convert():
    """Upload .blend oder .zip Datei und starte Konvertierung."""
    ...
    if not fname_lower.endswith(".blend") and not fname_lower.endswith(".zip"):
        return jsonify({"error": "Nur .blend oder .zip Dateien erlaubt"}), 400
    ...
    if fname_lower.endswith(".zip"):
        def convert_zip():
            cleanup_dir = None
            try:
                blend_path = extract_zip(upload_path, job_id)
                cleanup_dir = os.path.join(WORK_DIR, job_id)
                run_conversion(job_id, blend_path, cleanup_dir=cleanup_dir, options=options)
            except Exception as e:
                add_job_log(job_id, f"FEHLER: {e}")
                update_job(job_id, status="error", error=str(e))
            finally:
...
def convert_local_file():
    """Konvertiere eine lokale .blend Datei (Pfad als JSON)."""
    data = request.get_json(silent=True) or {}
    blend_path = data.get("path", "")

    if not blend_path or not os.path.exists(blend_path):
        return jsonify({"error": f"Datei nicht gefunden: {blend_path}"}), 400

    if not blend_path.lower().endswith(".blend"):
        return jsonify({"error": "Nur .blend Dateien erlaubt"}), 400

    # Copy file to uploads
    safe_name = os.path.basename(blend_path).replace(" ", "_")
    job_id = create_job(safe_name)
    upload_path = os.path.join(UPLOAD_DIR, f"{job_id}_{safe_name}")
    shutil.copy2(blend_path, upload_path)

    add_job_log(job_id, f"Lokale Datei: {blend_path}")
...
@app.route("/blend")
def list_blend():
    """Liste aller ZIP-Dateien im blend/ Ordner."""
    files = []
    for f in os.listdir(BLEND_DIR):
        if f.lower().endswith(".zip"):
            full = os.path.join(BLEND_DIR, f)
...
def scan_blend_folder():
    """blend/ Ordner scannen und Dateien auflisten (ohne zu konvertieren)."""
    files = []
...
def convert_folder():
    """Konvertiere ausgewaehlte oder alle Dateien aus blend/ Ordner."""
    data = request.get_json(silent=True) or {}
    options = {
    ...
    if not os.path.exists(BLEND_DIR):
        return jsonify({"error": "blend/ Ordner nicht gefunden"}), 400

    started = 0
    skipped = 0
    for fname in os.listdir(BLEND_DIR):
        ...
        zip_path = os.path.join(BLEND_DIR, fname)
        safe_name = fname.replace(" ", "_")
        job_id = create_job(safe_name)
        add_job_log(job_id, f"Aus blend/ Ordner: {fname}")

        def do_convert(zp=zip_path, jid=job_id, opts=options):
            cleanup_dir = None
            try:
                blend_file = extract_zip(zp, jid)
                cleanup_dir = os.path.join(WORK_DIR, jid)
                run_conversion(jid, blend_file, cleanup_dir=cleanup_dir, options=opts)
            except Exception as e:
...
<title>Blend &rarr; VRM Converter</title>
<style>
  :root {
    --bg: #0f172a;
    --bg2: #1e293b;
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .upload-area:hover, .upload-area.dragover { border-color:var(--accent); background:rgba(124,58,237,0.1); }
...
<body>
<div class="container">
  <h1>&#127917; Blend <span>&rarr;</span> VRM</h1>
  <p class="subtitle">Konvertiere Blender-Modelle zu VRM mit Gesichtsausdruecken, Bones &amp; Animationen (VSeeFace kompatibel)</p>
  ...
  <!-- Tabs -->
  <div class="tabs">
    <div class="tab active" onclick="switchTab('folder')">&#128194; blend/ Ordner</div>
  ...
  <div class="tab-content active" id="tab-folder">
    ...
    <div class="file-list" id="folderFileList">
      <div class="file-list-empty">Klicke "Ordner aktualisieren" um blend/ zu scannen</div>
    </div>
  </div>
  ...
  <div class="tab-content" id="tab-upload">
    <div class="upload-area" id="uploadArea" onclick="document.getElementById('fileInput').click()">
      <div class="icon">&#128193;</div>
      <p><strong>.zip oder .blend Datei hierhin ziehen</strong> oder klicken</p>
      <p style="font-size:0.85rem; margin-top:0.5rem;">ZIP = .blend + Texturen &bull; Max. 500 MB</p>
      <input type="file" id="fileInput" accept=".blend,.zip">
    </div>
  </div>
...
function renderFolderFiles() {
  ...
  if (folderFiles.length === 0) {
    list.innerHTML = '<div class="file-list-empty">Keine ZIP-Dateien in blend/ gefunden.</div>';
...
uploadArea.addEventListener('drop', function(e) {
  ...
  if (file && (file.name.endsWith('.blend') || file.name.endsWith('.zip'))) uploadFile(file);
  else alert('Bitte eine .blend oder .zip Datei auswaehlen');
});
...
if __name__ == "__main__":
    print(f"[Blend->VRM] Server: http://localhost:{SERVER_PORT}")
    print(f"[Blend->VRM] Blender: {BLENDER_PATH}")
    print(f"[Blend->VRM] Blender gefunden: {os.path.exists(BLENDER_PATH)}")
    print(f"[Blend->VRM] blend/ Ordner: {BLEND_DIR}")
    print(f"[Blend->VRM] vrm/ Ordner:   {VRM_DIR}")
    print(f"[Blend->VRM] Keine Auto-Konvertierung - starte manuell ueber Web-UI")
